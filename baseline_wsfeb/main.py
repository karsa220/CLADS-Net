...
# ==========================================
# 1. 核心创新：二维 Haar 小波变换算子
# ==========================================
class Haar2D(nn.Module):
    def __init__(self):
        super(Haar2D, self).__init__()
        ll = torch.tensor([[0.5, 0.5], [0.5, 0.5]])
        hl = torch.tensor([[-0.5, -0.5], [0.5, 0.5]])
        lh = torch.tensor([[-0.5, 0.5], [-0.5, 0.5]])
        hh = torch.tensor([[0.5, -0.5], [-0.5, 0.5]])
        self.filters = torch.stack([ll, hl, lh, hh]).unsqueeze(1)

    def forward(self, x):
        b, c, h, w = x.shape
        x_reshaped = x.view(b * c, 1, h, w)
        weight = self.filters.to(x.device).type_as(x)
        out = F.conv2d(x_reshaped, weight, stride=2)
        out = out.view(b, c, 4, h // 2, w // 2)
        return out[:, :, 0], out[:, :, 1], out[:, :, 2], out[:, :, 3]


class InverseHaar2D(nn.Module):
    def __init__(self):
        super(InverseHaar2D, self).__init__()
        ll = torch.tensor([[0.5, 0.5], [0.5, 0.5]])
        hl = torch.tensor([[-0.5, -0.5], [0.5, 0.5]])
        lh = torch.tensor([[-0.5, 0.5], [-0.5, 0.5]])
        hh = torch.tensor([[0.5, -0.5], [-0.5, 0.5]])
    ...
    def forward(self, ll, hl, lh, hh):
        b, c, h, w = ll.shape
        x = torch.stack([ll, hl, lh, hh], dim=2).view(b * c, 4, h, w)
        weight = self.filters.to(ll.device).type_as(ll)
...
# 2. 小波空间特征增强块 (W-SFEB)
# ==========================================
class W_SFEB(nn.Module):
    def __init__(self, in_channels):
        super(W_SFEB, self).__init__()
        ...
        self.dwt = Haar2D()
        self.idwt = InverseHaar2D()

        self.high_freq_filter = nn.Sequential(
            nn.Conv2d(in_channels * 3, in_channels * 3, 1, groups=in_channels),
            nn.Sigmoid()
        )
        ...
        self.channel_att = nn.Sequential(
            ...
            # nn.BatchNorm2d(in_channels), # 防止 batch_size 导致报错，保持注释
            nn.Sigmoid()
        )

    def forward(self, x):
        I1 = self.conv1(x)
        ll, hl, lh, hh = self.dwt(I1)

        high_freqs = torch.cat([hl, lh, hh], dim=1)
        high_freqs_att = self.high_freq_filter(high_freqs)
        high_freqs_filtered = high_freqs * high_freqs_att
        hl_f, lh_f, hh_f = torch.chunk(high_freqs_filtered, 3, dim=1)

        reconstructed = self.idwt(ll, hl_f, lh_f, hh_f)
        reconstructed = self.conv_refine(reconstructed)

        gap_feat = self.gap(x)
        f_cc = self.channel_att(gap_feat)
        return reconstructed * f_cc + x


# ==========================================
# 3. 基础模块：标准卷积块 (速度极快，无 DCN/注意力)
# ==========================================
...
# ==========================================
# 4. 网络主体：DenseUNet + W-SFEB
# ==========================================
class DenseUNet_WSFEB(nn.Module):
    def __init__(self):
        super(DenseUNet_WSFEB, self).__init__()
        densenet = models.densenet121(pretrained=True)
        self.encoder = densenet.features
        self.ch1, self.ch2, self.ch3, self.ch4 = 64, 256, 512, 1024

        # 🌟 创新点：跳跃连接处使用 W-SFEB
        self.wsfeb1 = W_SFEB(self.ch1)
        self.wsfeb2 = W_SFEB(self.ch2)
        self.wsfeb3 = W_SFEB(self.ch3)
        self.wsfeb4 = W_SFEB(self.ch4)

        # 解码器恢复为最普通的 ConvBlock，确保控制变量
        self.up4 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.dec4 = ConvBlock(1024 + self.ch4, 512)

        self.up3 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.dec3 = ConvBlock(512 + self.ch3, 256)

        self.up2 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.dec2 = ConvBlock(256 + self.ch2, 128)

        self.up1 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.dec1 = ConvBlock(128 + self.ch1, 64)
    ...
    def forward(self, x):
        ...
        # 🌟 经过 W-SFEB 增强跳跃连接的特征
        skip1 = self.wsfeb1(e0)
        skip2 = self.wsfeb2(e2)
        skip3 = self.wsfeb3(e3)
        skip4 = self.wsfeb4(e4)
...
def main():
    ...
    # 划分数据集 (固定种子 42，确保与 Baseline 对比的一致性)
    ...
    train_loader = DataLoader(train_sub, batch_size=8, shuffle=True, drop_last=True)
    ...
    # 实例化引入了 W-SFEB 的模型
    model = DenseUNet_WSFEB().to(device)
    ...
    plt.title('Baseline + W-SFEB Training Progress')
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_dice > best_val_dice:
            ...
            # 命名为 wsfeb 权重以区分纯 baseline
            torch.save(model.state_dict(), "wsfeb_best_model.pth")
    ...
    plt.ioff()
    plt.savefig('wsfeb_training_curve.png', dpi=150)

    # --- 最终测试阶段 (Test) ---
    print("\n" + "=" * 50)
    print("🚀 开始在完全未见的【测试集 (Test Set)】上评估性能...")

    model.load_state_dict(torch.load("wsfeb_best_model.pth"))
    model.eval()
