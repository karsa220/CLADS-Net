...
# ==========================================
# 2. Baseline 网络结构：标准 Dense-UNet
# (移除 W-SFEB，移除 DCN，纯粹的 U-Net 跳跃连接)
# ==========================================
class DenseUNet_Baseline(nn.Module):
    def __init__(self):
        super(DenseUNet_Baseline, self).__init__()
        # 编码器 (Encoder)
        densenet = models.densenet121(pretrained=True)
        self.encoder = densenet.features
        self.ch1, self.ch2, self.ch3, self.ch4 = 64, 256, 512, 1024

        # 解码器 (Decoder) - 使用最普通的卷积块
    ...
    def forward(self, x):
        # --- 编码阶段 ---
        e0 = self.encoder.relu0(self.encoder.norm0(self.encoder.conv0(x)))  # ch: 64
        e2 = self.encoder.denseblock1(self.encoder.pool0(e0))  # ch: 256
        e3 = self.encoder.denseblock2(self.encoder.transition1(e2))  # ch: 512
        e4 = self.encoder.denseblock3(self.encoder.transition2(e3))  # ch: 1024

        # 瓶颈层特征
        b = self.encoder.norm5(self.encoder.denseblock4(self.encoder.transition3(e4)))  # ch: 1024

        # --- 解码阶段 (纯粹的 Skip Connections，没有任何注意力或小波增强) ---
...
class BUSIDataset(Dataset):
    def __init__(self, data_dir):
        ...
        categories = ['benign']  # 与改进模型保持一致的测试基准
...
def main():
    ...
    # 划分数据集 (7:1:2 固定随机种子，确保与你的改进模型使用完全一样的数据)
    ...
    print(f"✅ Baseline 数据加载完毕 | Train: {train_size} | Val: {val_size} | Test: {test_size}")

    # 这里实例化的是 Baseline 模型
    model = DenseUNet_Baseline().to(device)
    ...
    ax1.set_ylabel('Train Loss', color='tab:gray')
    ax2.set_ylabel('Validation Dice', color='tab:purple')
    plt.title('Baseline Training Progress')

    line_loss, = ax1.plot([], [], color='tab:gray', marker='o', label='Baseline Train Loss')
    line_dice, = ax2.plot([], [], color='tab:purple', marker='s', label='Baseline Val Dice')
    ...
    for epoch in range(num_epochs):
        ...
        pbar_train = tqdm(train_loader, desc=f"Epoch [{epoch + 1}/{num_epochs}] Baseline Train", leave=False)
        ...
        print(
            f"📉 Epoch {epoch + 1} | Baseline Train Loss: {avg_train_loss:.4f} | Baseline Val Dice: {avg_val_dice:.4f}")
        ...
        if avg_val_dice > best_val_dice:
            ...
            # 注意保存的文件名改了，以免覆盖你改进模型的权重
            torch.save(model.state_dict(), "baseline_best_model.pth")
            print(f"🌟 [New Baseline Best] Val Dice: {best_val_dice:.4f}")
    ...
    print("\n🏆 Baseline 最终测试集成绩 🏆")
    print(f"Baseline Test Dice: {np.mean(test_dices):.4f}")
    print(f"Baseline Test IoU:  {np.mean(test_ious):.4f}")
    print("=" * 50)
