...
class Basic_FEB_ReLU(nn.Module):
    """特征增强模块：使用标准 ReLU"""

    def __init__(self, in_channels):
        super(Basic_FEB_ReLU, self).__init__()
...
# ==========================================
# 2. 网络主体：HANet + SegFormer MLP Decoder
# ==========================================

class HANet_MLP_ReLU(nn.Module):
    def __init__(self, embed_dim=256):
        super(HANet_MLP_ReLU, self).__init__()
        # Encoder: DenseNet121
        ...
        # 特征增强模块
        self.feb2 = Basic_FEB_ReLU(self.ch2)
        self.feb3 = Basic_FEB_ReLU(self.ch3)

        # Decoder：将四个阶段特征投影到统一维度
    ...
    def forward(self, x):
        # 1. Encoder 路径 (修正后的完整逻辑)
        e0 = self.encoder.relu0(self.encoder.norm0(self.encoder.conv0(x)))  # 1/2
        e2 = self.encoder.denseblock1(self.encoder.pool0(e0))  # 1/4
        e3 = self.encoder.denseblock2(self.encoder.transition1(e2))  # 1/8

        # 必须经过 denseblock3 和 transition2 才能到达 1024 通道
        e4 = self.encoder.denseblock3(self.encoder.transition2(e3))  # 1/16 (1024ch)
        b = self.encoder.norm5(self.encoder.denseblock4(self.encoder.transition3(e4)))  # 1/32
        ...
        # 3. 多尺度特征对齐 (All-MLP)
        target_size = e0.size()[2:]  # 对齐到 1/2 尺寸
...
def main():
    ...
    # 实例化新模型
    model = HANet_MLP_ReLU(embed_dim=256).to(device)
