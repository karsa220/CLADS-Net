...
from my_model_busi import HybridLoss
...
def main():
    ...
    pretrained_weights = "best_model_mlp_decoder.pth"  # BUSI 上的预训练权重路径
    ...
    # 2. 加载预训练权重 (Cross-Dataset Transfer 的关键)
    if os.path.exists(pretrained_weights):
        print(f"🔄 正在加载 BUSI 预训练权重: {pretrained_weights}")
        # load_state_dict 会将预训练参数完整映射到当前模型
        model.load_state_dict(torch.load(pretrained_weights, map_location=device))
        print("✅ 预训练权重加载成功！开始基于先验知识学习。")
    else:
        print(f"❌ 找不到预训练权重 {pretrained_weights}，请确认路径！")
        return
    if MODE == "train":
